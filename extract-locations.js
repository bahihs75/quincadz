...
// Paste the full script from your message here (the part containing wilaya and communes arrays)
const scriptContent = `<script>
    var nameNumber = "extra_fields[custom_field_XtNXuoxG9uppikZi]";
    var nameWilaya = "extra_fields[custom_field_rcWpsS2JD14cRgzj]";
    var nameCommune = "extra_fields[custom_field_CvIV1FggnC5RQbxk]";
    var stoppdesk = "extra_fields[custom_field_c7mBPc8bBtPMUI4w]";
...
// Extract wilaya array
const wilayaMatch = scriptContent.match(/wilaya\s*=\s*\[([\s\S]*?)\];/);
if (!wilayaMatch) throw new Error('Wilaya array not found');
const wilayaLines = wilayaMatch[1].split(',').map(s => s.trim().replace(/^"|"$/g, '')).filter(s => s);
const wilayas = wilayaLines.map((line, index) => {
  const [code, nameAr] = line.split('~').map(s => s.trim());
  return {
    id: index + 1,
    code,
    name_ar: nameAr,
...
// Extract commune mapping (from the change event handlers)
const communeMatches = scriptContent.matchAll(/if \("([^"]+)" == a\) \{\s*communes = \[([\s\S]*?)\];/g);
const baladiyas = [];
let baladiyaId = 1;
for (const match of communeMatches) {
  const wilayaName = match[1].split('~')[1].trim();
  const wilaya = wilayas.find(w => w.name_ar === wilayaName);
  if (!wilaya) continue;
  const communeLines = match[2].split(',').map(s => s.trim().replace(/^"|"$/g, '')).filter(s => s);
  for (const nameAr of communeLines) {
    baladiyas.push({
      id: baladiyaId++,
      wilaya_id: wilaya.id,
      name_ar: nameAr,
...
const output = `// src/lib/algeriaData.ts
export const wilayas = ${JSON.stringify(wilayas, null, 2)};

export const baladiyas = ${JSON.stringify(baladiyas, null, 2)};
`;

fs.writeFileSync('src/lib/algeriaData.ts', output);
console.log('✅ src/lib/algeriaData.ts created');
