...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (email !== confirmEmail) {
      toast.error('البريدان الإلكترونيان غير متطابقين')
      return
    }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border border-slate-100 p-8">
        <div className="text-center mb-8">
          <img src="/brand.svg" alt="QuincaDZ" className="h-12 mx-auto mb-4" />
          <h1 className="text-2xl font-bold text-slate-800">QuincaDZ</h1>
          <p className="text-slate-500 text-sm mt-1">إنشاء حساب جديد</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">تأكيد البريد الإلكتروني</label>
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
              <input
                type="email"
        ...
        <p className="mt-6 text-center text-sm text-slate-600">
          لديك حساب بالفعل؟{' '}
          <Link href="/auth/login" className="text-primary hover:text-secondary font-medium">
            تسجيل الدخول
          </Link>
        </p>
      </div>
